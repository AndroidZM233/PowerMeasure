...
public class SpecialSecondAct extends FragActBase {
    @ViewById
    CustomTitlebar titlebar;
    @ViewById
    Button btn_spSecond_fankui;
    @ViewById
    Button btn_spSecond_change;
    @ViewById
    ...
    protected void main() {
        ...
        App.getInstance().addActivity(SpecialSecondAct.this);
        initTitlebar();
        setSwipeEnable(false);

        tv_spSecond_time.setText(getXml("END_TIME",""));
        tv_spSecond_start_time.setText(getXml("START_TIME",""));
        String file_name = getXml("FILE_NAME", "test.txt");
        String file_path = getXml("FILE_PATH", "/data");
    ...
    void tv_spSecond_fujian(){
        openAct(AttachmentAct.class,true);
    }
    //从FTP下载
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                boolean initFTPSetting = ftpUtils
                        .initFTPSetting("192.168.1.61", 22, "ftpiom", "ftpiom");
                if (initFTPSetting){
                    ftpUtils.downLoadFile("/data/data/speedata.com.powermeasure/files/"+file_name
                            ,file_name,file_path,"B_");
                }
    ...
    void btn_spSecond_fankui(){
        ...
        },getXml("OPER_NO", "1"),getXml("ORDER_NO","1"),et_spSecond_trouble.getText().toString());
