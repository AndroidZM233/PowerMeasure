package speedata.com.powermeasure.HorizScroll;
...
public class HorizontalScrollMenu extends LinearLayout
{
	...
	private ColorStateList mColors,mColorsQB;
			...
			int defStyle)
	{
		...
		mColorsQB=getResources().getColorStateList(
				R.drawable.selector_menu_item_text2);
	...
	public void notifyDataSetChanged(BaseAdapter adapter)
	{
		...
		int location=SharedXmlUtil.getInstance(mContext).read("location", 0);
		try {
			clickItem(location);
		} catch (Exception e) {
			clickItem(0);
...
//					rb_item.setGravity(Gravity.CENTER);
					...
					rb_item.setText(str);
					rb_item.setGravity(Gravity.CENTER);
					rb_item.setButtonDrawable(R.drawable.danyuan);
					rb_item.setPadding(mPaddingLeft, mPaddingTop, mPaddingRight,
							mPaddingBottom);
					rg_items.addView(rb_item);
					rb_items.add(rb_item);
...
//			rb_items.get(0).setChecked(true);
	 ...
	 * @param location
     */
	public void clickItem(int location){
		rb_items.get(location).setChecked(true);
//		moveItemToCenter(rb_items.get(location));
		moveItemToCenterByLocation(location);
	}
	/**
	 * 菜单项切换监听器
	 */
	...
	private OnPageChangeListener mPageListener = new OnPageChangeListener()
	{
		...
		public void onPageSelected(int arg0)
		{
			...
			SharedXmlUtil.getInstance(mContext).write("location", arg0);
		}
	...
	public void moveItemToCenterByLocation(int location){
		if (location>2){
			hsv_menu.smoothScrollBy(90*location,
					0);
		}
